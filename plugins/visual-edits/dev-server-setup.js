...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all Zomant.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*Zomant\.sh$/)) {
        return true;
      }

      // Allow all Zomantagent.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*Zomantagent\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@Zomant.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@Zomant.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
